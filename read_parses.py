...
def extract_parses(fname):
    ...
    meet_first_sentence = False
    for idx, line in tqdm(enumerate(f)):
        # Use flag instead of line number to determine.
        # The output formats of different version of CoreNLP are different.
        if (not meet_first_sentence) and (not line.startswith('Sentence #')):
            continue  # skip until meeting the first normal sentence
        if line.startswith('Sentence #'):
            new_sent = True
            new_pos = False
            new_parse = False
            new_deps = False
            if not meet_first_sentence:
                meet_first_sentence = True
                continue  # the first sentence has not been processed
...
def write_parses(parses, writer):
    for idx, parse in tqdm(enumerate(parses)):
...
if __name__ == '__main__':
    ...
    if not args.input.endswith(".out"):
        raise ValueError("The input file should be a `.out` file generated by the Stanford CoreNLP Java Program.")

    para_count = 0

    # write fieldnames
    fn = ['tokens', 'parse']
    ofile = codecs.open(args.output, 'w', 'utf-8')
    out = csv.DictWriter(ofile, delimiter='\t', fieldnames=fn)
    out.writerow(dict((x,x) for x in fn))

    parses = extract_parses(args.input)
    print(len(parses))
